...
### AI-11: Cognitive Layering — 认知分层

**含义**: 高级模型的 context 是稀缺资源。数据采集、搜索、摘要、格式转换等"苦力活"应交给确定性脚本或低成本模型，高级模型只消费精简后的结构化输入，专注判断和推理。

**与 AI-07 的关系**: AI-07 说"Token 是核心资源"，AI-11 说"怎么让每个 token 都是高价值信息"。

**分层架构**:

| 层 | 职责 | 工具 |
|----|------|------|
| 判断层 (高级模型) | 推理、决策、架构设计 | 对话 |
| 预处理层 (脚本) | 搜索、阅读 URL、批量摘要、格式转换 | Python 脚本 |
| 采集层 (低成本模型/API) | 大量文本的摘要、翻译、信息提取 | Gemini Flash / GPT-4o-mini |

**信息源优先级**: 造轮子前先找轮子。高价值信息源优先搜索：GitHub → HuggingFace → 官方文档 → 技术博客 → 通用搜索。

**Context 节省估算**:

| 操作 | 传统方式 | 分层方式 | 节省 |
|------|---------|---------|------|
| 3 条搜索 | ~6000 tokens 注入 context | 脚本输出 ~500 tokens | **12x** |
| 读 5 篇文章 | ~25000 tokens | 脚本摘要 ~2000 tokens | **12x** |
| 启动读 4 个文件 | ~4000 tokens | 脚本生成状态摘要 ~500 tokens | **8x** |

**实施工具**: `_ai_evolution/scripts/search.py` (批量搜索 + 精简输出)

**出现记录**: 2026-02-13 讨论 OpenClaw + token 效率
